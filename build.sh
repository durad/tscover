...
SRC_CMD="$SRC_CMD --outDir \"$OUTDIR_NORM\" "
...
if [[ $WATCH == false ]]; then
	...
	export FINALCMD="concurrently --kill-others --prefix \"[{name}]\" --names \"SRC_,HEAD\""
	FINALCMD="$FINALCMD -c \"bgGreen.black,bgYellow.black\""
	...
	FINALCMD="$FINALCMD \"$SRC_CMD\" \"$HEADER_CMD\" "
	eval "$FINALCMD"
fi
